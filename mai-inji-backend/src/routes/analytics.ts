  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        by_day: byDayResult.rows.map((row: any) => ({
          date: row.date,
        ...
        top_items: topItemsResult.rows.map((row: any) => ({
 ...
 */
router.get('/items/popular', async (req: Request, res: Response) => {
  try {
 ...
 */
router.get('/revenue/category', async (req: Request, res: Response) => {
  try {
    const result = await pool.query(`
      SELECT 
        mc.id,
